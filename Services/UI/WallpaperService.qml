...
Singleton {
  ...
  // -------------------------------------------------------------------
  // Clear monitor wallpaper cache (lists, active wallpaper, used random list, and saves to file)
  function clearMonitorWallpaperCache(screenName) {
    ...
    // Clear current wallpaper entry
    if (currentWallpapers[screenName] !== undefined) {
      delete currentWallpapers[screenName];
      var cw = currentWallpapers;
      currentWallpapers = cw;
    }

    // Reset used random tracking
  ...
  // -------------------------------------------------------------------
  property int _refreshRetryAttempts: 0
  function refreshWallpapersList() {
    ...
    if (!ImageCacheService.initialized) {
      if (_refreshRetryAttempts < 20) {
        _refreshRetryAttempts++;
        Qt.callLater(refreshWallpapersList);
      } else {
        Logger.w("Wallpaper", "ImageCacheService init timed out after 20 attempts, proceeding anyway");
  ...
  Timer {
    ...
    running: Settings.data.wallpaper.automationEnabled && !PowerProfileService.quickislandPerformanceMode
    repeat: true
  ...
  FileView {
    ...
    adapter: JsonAdapter {
      ...
      property string defaultWallpaper: root.quickislandDefaultWallpaper
      property var usedRandomWallpapers: ({})
    }
    ...
    onLoaded: {
      ...
      // Load default wallpaper from cache if it exists, otherwise use Quickisland default
      if (wallpaperCacheAdapter.defaultWallpaper && wallpaperCacheAdapter.defaultWallpaper !== "") {
        ...
        root.defaultWallpaper = root.quickislandDefaultWallpaper;
        Logger.d("Wallpaper", "Using Quickisland default wallpaper");
      }

      Logger.d("Wallpaper", "Loaded wallpapers from cache file:", Object.keys(root.currentWallpapers).length, "screens");
      root.isInitialized = true;
      
      // Apply cached wallpapers via awww at startup
      Object.keys(root.currentWallpapers).forEach(function (screenName) {
