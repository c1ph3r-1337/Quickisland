...
Singleton {
  ...
  // -------------------------------------------------
  Loader {
    id: fallbackRendererLoader
    active: !root.imageMagickAvailable || root.fallbackProcessing
    asynchronous: false
    sourceComponent: PanelWindow {
      ...
      property alias fallbackImage: fallbackImage
  ...
  readonly property var fallbackImage: fallbackRendererLoader.item ? fallbackRendererLoader.item.fallbackImage : null
  ...
  function queueFallbackProcessing(sourcePath, destPath, cacheKey, size) {
    ...
    if (!fallbackProcessing) {
      ...
      const img = root.fallbackImage;
      if (img) {
        img.cacheKey = item.cacheKey;
        img.destPath = item.destPath;
        img.targetSize = item.size;
        img.source = item.sourcePath;
      }
    }
  }
