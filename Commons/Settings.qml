...
Singleton {
  ...
  property int upgradeAttempts: 0
  ...
  function upgradeSettings() {
    ...
    if (!PluginService.initialized || !PluginService.pluginsFullyLoaded) {
      if (upgradeAttempts < 20) {
        upgradeAttempts++;
        Logger.d("Settings", "Plugins not fully loaded yet, deferring upgrade (attempt " + upgradeAttempts + ")");
        Qt.callLater(upgradeSettings);
      } else {
        Logger.w("Settings", "Plugins loading timed out, skipping upgrade checks");
      }
      return;
    }
    ...
    if (!BarWidgetRegistry.widgets || Object.keys(BarWidgetRegistry.widgets).length === 0) {
      if (upgradeAttempts < 20) {
        upgradeAttempts++;
        Logger.d("Settings", "BarWidgetRegistry not ready, deferring upgrade (attempt " + upgradeAttempts + ")");
        Qt.callLater(upgradeSettings);
      } else {
        Logger.w("Settings", "BarWidgetRegistry loading timed out or not used, skipping upgrade checks");
      }
      return;
    }
