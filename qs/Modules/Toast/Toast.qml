...
Item {
  ...
  readonly property bool isColorType: /^#([0-9a-fA-F]{3,4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/.test(root.type) || root.type.startsWith("rgb") || root.type.startsWith("hsl")
  ...
  Rectangle {
    ...
    border.color: {
      if (root.isColorType) {
        return Qt.alpha(root.type, Color.adaptiveOpacity(Settings.data.notifications.backgroundOpacity) || 1.0);
      }
      var baseColor;
      switch (root.type) {
      case "error":
