...
Singleton {
  ...
  readonly property real batteryPercentage: {
    if (!primaryDevice) return -1;
    var _prop1 = primaryDevice.percentage !== undefined ? primaryDevice.percentage : 0;
    var _prop2 = primaryDevice.battery !== undefined ? primaryDevice.battery : 0;
    return getPercentage(primaryDevice);
  }
  readonly property bool batteryCharging: {
    if (!primaryDevice) return false;
    var _prop1 = primaryDevice.state !== undefined ? primaryDevice.state : 0;
    return isCharging(primaryDevice);
  }
  readonly property bool batteryPluggedIn: {
    if (!primaryDevice) return false;
    var _prop1 = primaryDevice.state !== undefined ? primaryDevice.state : 0;
    return isPluggedIn(primaryDevice);
  }
  readonly property bool batteryReady: {
    if (!primaryDevice) return false;
    var _prop1 = primaryDevice.ready !== undefined ? primaryDevice.ready : false;
    var _prop2 = primaryDevice.batteryAvailable !== undefined ? primaryDevice.batteryAvailable : false;
    return isDeviceReady(primaryDevice);
  }
  readonly property bool batteryPresent: {
    if (!primaryDevice) return false;
    var _prop1 = primaryDevice.isPresent !== undefined ? primaryDevice.isPresent : false;
    var _prop2 = primaryDevice.connected !== undefined ? primaryDevice.connected : false;
    return isDevicePresent(primaryDevice);
  }
