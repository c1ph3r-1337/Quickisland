...
ShellRoot {
  ...
  property var profileSelectorConnection: null

  function showProfileSelector() {
    var targetScreen = PanelService.findScreenForPanels();
    if (!targetScreen) {
      Logger.w("Shell", "No screen available to show profile selector");
      showWizardOrChangelog();
      return;
    }

    var selectorPanel = PanelService.getPanel("profileSelectorPanel", targetScreen);
    if (!selectorPanel) {
      Logger.w("Shell", "Profile selector panel not ready");
      showWizardOrChangelog();
      return;
    }
